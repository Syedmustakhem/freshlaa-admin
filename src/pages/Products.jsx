...
quickFilter: editProduct.quickFilter || null,      }, { headers: authHeader() });
  ...
  /* ── Variant row UI (reused in both modals) ── */
                    ...
                    <td style={{ padding: "12px 14px", fontWeight: 700, color: "#111827" }}>
                      {def ? (
                        <>
                          ₹{def.price}
                          {def.mrp > def.price && (
                            <span style={{ marginLeft: 6, fontSize: 11, color: "#9ca3af", textDecoration: "line-through" }}>₹{def.mrp}</span>
                          )}
                        </>
                      ) : "—"}
                    </td>
      ...
      <option value="vegetables">🥦 Vegetables</option>
          ...
          <Section icon="📂" title="Category Selection">
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <Input placeholder="https://…" value={subCategoryImage} onChange={e => setSubCategoryImage(e.target.value)} />
            </div>
          </Section>
      ...
      <option value="vegetables">🥦 Vegetables</option>
          ...
          <Section icon="🖼" title="Product Images">
            {(editProduct.images || []).map((img, i) => (
              <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                <Input placeholder="https://…" value={img}
